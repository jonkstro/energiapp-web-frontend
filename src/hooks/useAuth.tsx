    ...
    if (validarDadosCadastro()) {
      await api
        ...
        .then((response) => {
          ...
          notifySuccess("Enviamos um email para ativar sua conta.");
  ...
  async function forgotPassword(emailInput: Forgot) {
    if (email.length > 6) {
      await api
        ...
        .then((response) => {
          notifySuccess("Enviamos um email com o link de alteração de senha");
        })
        .catch((error) => {
          console.log(error);
          notifyErrors("Erro interno do sistema");
  ...
  async function resetPassword(resetInput: ResetPassword) {
    if (
      ...
      await api
        ...
        .then((response) => {
          notifySuccess("Alterado a senha com sucesso");
          // router.push("/auth");
        })
        .catch((error) => {
          console.log(error);
